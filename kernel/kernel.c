...
void kernel_main(multiboot_info_t* mbt, unsigned int magic) {
    ...
    terminal_writestring(
        "Hello, kernel World! (IDT) \nHello again!! And one more time!\n");

    printf("Printing 12345: %d\n", 12345);
    printf("Printing 0xdeadbeef: 0x%x\n", 0xdeadbeef);
    printf("Magic is: 0x%x\n", magic);
    const unsigned int memmap_offset =
        (unsigned int)&_kernel_physical_memmap - (unsigned int)mbt;
    ...
    puts("addr and len as 64-bit integers:");
        while(entry < (mmap_entry_t*)((mbt->mmap_addr + memmap_offset) +
    mbt->mmap_length)) {
    ...
    */
    ...
    // This function also enables PIC interrupts
